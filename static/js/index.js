...
class BoolControl extends Rete.Control {
  ...
  handler(el, editor) {
    var text = this.getData(this.key) || "input";
...
class DataProcessorComponent extends Rete.Component {
  constructor() {
  ...
  builder(node) {
    ...
    for (var p in node.data.input) {
      var inputp = node.data.input[p];
      var t = inputp.name;
      var inp = new Rete.Input(inputp.name, inputp.type, strSocket);
      node.addInput(inp);
    }
    for (var p in node.data.output) {
      var output = node.data.output[p];
      var out = new Rete.Output(output.name, output.type, strSocket);
      node.addOutput(out);
    }
    for (var p in node.data.param) {
      var param = node.data.param[p];
      if (param[1] == "BooleanParam") {
        var input_param = new BoolControl(param[0]);
        node.addControl(input_param);

      } else {
        var input_param = new InputControl(param[0])
        node.addControl(input_param);
      }
    }

    var node_name = new TextControl(node.data.text, node.data.operator);
    node.addControl(node_name);
